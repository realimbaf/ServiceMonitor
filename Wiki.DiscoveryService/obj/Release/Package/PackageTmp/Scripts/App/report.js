...
define(['knockout', 'jquery', 'underscore', 'service', 'config/config', 'config/menu', 'tooltip'], function (ko, $, _, service, config, menu, tooltip) {
    ...
    function convertSimStatus(status) {
        ...
        switch (status) {
            case "modem_init_complete":
                result = $.i18n.prop("main_sim_status_ins");//$.i18n.prop("sim_status_ready");
                break;
            case "modem_waitpin":
                result = $.i18n.prop("main_sim_status_invalid");//$.i18n.prop("sim_status_waitpin");
                break;
            case "modem_waitpuk":
                result = $.i18n.prop("main_sim_status_invalid");//$.i18n.prop("sim_status_waitpuk");
                break;
            case "modem_sim_undetected":
                result = $.i18n.prop("main_sim_status_no");//$.i18n.prop("sim_status_undetected");
                break;
            case "modem_undetected":
                result = $.i18n.prop("main_sim_status_invalid");
                break;
            case "modem_imsi_waitnck":
                result = $.i18n.prop("main_sim_status_invalid");//$.i18n.prop("sim_status_waitnck");
                break;
            case "modem_sim_destroy":
                result = $.i18n.prop("main_sim_status_invalid");//$.i18n.prop("sim_status_destroy");
                break;
            case "modem_destroy":
                result = $.i18n.prop("main_sim_status_invalid");//$.i18n.prop("sim_status_destroy");
                break;
        ...
        var getOTAUpgradeState = function () {
            ...
            if (_otaUpdateCancelFlag) {
                if (state == 'connecting_server') {
                    ...
                    showCustomInfo('ota_download_success');
                    window.clearTimeout(timer);
                    return;
    ...
    function showOTAConfirm(upgradeState) {
        if (upgradeState.current_upgrade_state == 'upgrade_pack_redownload') {
            ...
            showConfirm($.i18n.prop('ota_new_version') + version, {
                ok: function () {
                    getUserSelector(1);
                }, no: function () {
                    getUserSelector(0);
